...
const Index = () => {
	...
	const { data /*, fetch */ } = app.useFetch((ctx: App.Context) => {
		return ctx.rpc.getHelloWorld(ctx.rpc.options({ ttlSeconds: 0 }));
	});

	const stream = app.useFetch(
		(ctx: App.Context) => {
			return ctx.rpc.getHelloWorldAsStream.asResponse(ctx.rpc.options({ ttlSeconds: 0 }));
		},
		{
			shouldFetch: ({ worker }) => {
				return !worker;
	...
	return (
		<div className='flex h-screen w-full items-center justify-center bg-gray-900'>
			<div className='space-y-3 text-center'>
				...
				<pre className='rounded-md bg-gray-950 p-2 text-left text-white'>{JSON.stringify(data, null, 2)}</pre>
